  ...
  def compileTerm(self):
    ...
    if daToken == '<identifier>':
      ...
      if lookAhead == '<symbol> [ </symbol>\n': # then it is an array
        ...
        #self.writeAdv() # '=' this is not needed because it is done
        #self.compileExpression() this is not needed because only array here
      ...
      if self.tTp[self.tokenCounter-1].split()[1] == '=':
        arrayVMToken = VMToken.split()
        for i in range(len(arrayVMToken)):
          self.theVMWriter.writeCall('String.appendChar',arrayVMToken[i])
      else:
        self.theVMWriter.writePush('constant',VMToken)
        self.theVMWriter.writeCall('String.new',len(VMToken))
      self.writeAdv()
